...
Gen1<-ggplot(Resumen_G1, aes(x=Treatment, y=Mean_Expresion, fill=Treatment)) + 
...
Prom_Int_C2<-Ctdata%>%
  ...
  filter(Treatment=="Control", Tissue=="Cerebro")%>%
  summarize(Prom_Int_C2=mean(Gen2))

class(Prom_Int_C2)

Prom_Int_C2<-as.numeric(Prom_Int_C2)

# Normalizar el CT del gen de interes al del gen de referencia para muestra control  
GR_GI2<-(Prom_Int_C2-Prom_Ref_C)
GR_GI2


# Calculo Expresion
ExpresionG2<-Ctdata%>%
  select(Replica,Treatment,Tissue,Gen_ref, Gen2, Eficiencia)%>%  #Selecciona Gen 1
  filter(Treatment %in% c("Control","A","B","C"))%>% # Selecciona tratamientos, el control queda con valor 1 
  mutate(GR_GI_T2=Gen2-Gen_ref)%>% # GR_GI_T= Resta del Ct del gen de referncia al Ct del gen de interes por cada muestra
  mutate(Trat_ctrl2=GR_GI_T2-GR_GI2)%>% # Resta de GR_GI de los controles al GR_GI_T de los tratamientos por cada muestra            
  mutate(Expresion2=2^-(Trat_ctrl2)) # 2^Delta-Delta CT= Relacion de expresion normalizada

# Promedios y desviaciones estandar por tratamiento

Resumen_G2 <- ExpresionG2 %>% select(Treatment, Expresion2) %>% group_by(Treatment) %>%
  summarize(n=n(),
            Mean_Expresion2=mean(Expresion2),
            sd_Expresion2=sd(Expresion2))

# Grafico de barras Fold-change

Gen2<-ggplot(Resumen_G2, aes(x=Treatment, y=Mean_Expresion2, fill=Treatment)) + 
  geom_bar(stat="identity", color="black", position=position_dodge()) +
  geom_errorbar(aes(ymin=Mean_Expresion2-sd_Expresion2, ymax=Mean_Expresion2+sd_Expresion2), width=.2,
                position=position_dodge(.9))+
  theme(legend.position="none")+
  labs(y="Fold Change (2^ddCt)",title="Gen 2")
